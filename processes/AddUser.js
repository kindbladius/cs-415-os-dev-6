...
// NEW Register process now contains a Manual Property that takes a string which will be displayed
  ...
  os.bin.AddUser = AddUser;
  os.ps.register('AddUser', AddUser, {stdin: AddUserListener, stdout: true, pipeIn: false},userMan);

  function AddUser(options, argv) {
    ...
    async.waterfall([
      ...
      if (error===-1)
        console.log('AddUser: ERROR in execution. exited early');
    });
  }
/*
  ...
  */
  ...
  function search(substr, key) {
	...
	function AddUserListener(stream) {
		...
		async.waterfall([
			...
			function (fullResult, recursivecallback) {
				...
				if(pipeIn == true) {
					os._internals.ps.processTable['AddUser'].options.pipeIn = false;
				} else {
