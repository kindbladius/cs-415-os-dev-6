...
// NEW Register process now contains a Manual Property that takes a string which will be displayed
  ...
  os.bin.RemoveUser = RemoveUser;
  os.ps.register('RemoveUser', RemoveUser, {stdout: true, pipeIn: false},userMan);

  function RemoveUser(options, argv) {
      ...
      function (length, fh, fullData, callback) {
		...
		async.waterfall([
			...
			function (fullResult, callback) {
				...
				if(pipeIn == true) {
					os._internals.ps.processTable['RemoveUser'].options.pipeIn = false;
				} else {
			...
			if(pipeIn == true)
              ...
              if(err===-1){
				...
				if (error===-1)
					console.log('RemoveUser: ERROR in execution. exited early');
				else {
