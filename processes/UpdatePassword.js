...
// NEW Register process now contains a Manual Property that takes a string which will be displayed
  ...
  os.bin.UpdatePassword = UpdatePassword;
  os.ps.register('UpdatePassword', UpdatePassword, {stdin: UpdatePasswordListener, stdout: true, pipeIn: false},userMan);

  function UpdatePassword(options, argv) {
    ...
    async.waterfall([
      ...
      if (error===-1)
        console.log('UpdatePassword: ERROR in execution. exited early');
    });
  }
/*
  ...
  */
  ...
  function search(substr, key) {
	...
	function UpdatePasswordListener(stream) {
		...
		async.waterfall([
			...
			function (fullResult, recursivecallback) {
				...
				if(pipeIn == true) {
					os._internals.ps.processTable['UpdatePassword'].options.pipeIn = false;
				} else {
